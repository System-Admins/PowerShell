...
# Create new Win32 client app in Intune.
Function New-IntuneWin32App
{
    ...
    # Write to log.
    Write-Log ("Creating '{0}' client application in Intune" -f $DisplayName);

    # Create application in Intune.
    $Response = Invoke-RestMethod -Method Post -Headers $Headers -Body $Body -Uri "https://graph.microsoft.com/beta/deviceAppManagement/mobileApps" -ContentType "application/json";
...
Function Add-IntuneWin32App
{
    ...
    $IntuneWinOutput = ('{0}\{1}' -f (Get-File -Path $Application.Path -Option DirectoryName), (New-Guid).Guid)
    ...
    $IntuneWin32App = New-IntuneWin32App -ApiToken $ApiToken `
...
# Get graph token.
$ApiToken = Get-ApiToken -TenantId $AzureADApp.TenantId -ClientId $AzureADApp.ClientId -ClientSecret $AzureADApp.ClientSecret;

# Get all Win32 apps.
$Win32Apps = Get-IntuneMobileApps -ApiToken $ApiToken | Where-Object {$_."@odata.type" -like "*win32*"};

# Add new Win32 app.
#Add-IntuneWin32App -ApiToken $ApiToken -Application $Application;

# Replace Win32 app details and content.
#Replace-IntuneWin32App -ApiToken $ApiToken -AppId <id> -Application $Application;
